...
import { ServiceOrder, Expense, Unit, OSStatus, OSType, ExpenseCategory, Supplier, ExpenseStatus, User } from '../types';
...
interface ReportsProps {
  ...
  currentUser?: User; // Needed for audit log
  isMobile?: boolean;
}
...
export const Reports: React.FC<ReportsProps> = ({ orders, expenses, suppliers, isDarkMode = false, onOpenOS, onUpdateExpenses, currentUser, isMobile = false }) => {
  ...
  const toggleTableUnit = (unit: Unit) => { setTableUnits(prev => prev.includes(unit) ? prev.filter(u => u !== unit) : [...prev, unit] ); };
  const clearTableUnits = () => setTableUnits([]);
  const toggleSelection = (id: string) => { setSelectedExpenseIds(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id] ); };
  ...
  const pmaValue = useMemo(() => {
    ...
    const durations = closedOrders.map(o => { const start = new Date(o.dateOpened).getTime(); const end = new Date(o.dateClosed!).getTime(); return Math.ceil(Math.abs(end - start) / (1000 * 3600 * 24)); }).sort((a, b) => a - b);
  ...
  const statusData = useMemo(() => {
      ...
      return [ { name: 'Concluídas', value: counts[OSStatus.CONCLUIDA], color: COLORS_STATUS.CONCLUIDA }, { name: 'Em Andamento', value: counts[OSStatus.EM_ANDAMENTO], color: COLORS_STATUS.EM_ANDAMENTO }, { name: 'Abertas', value: counts[OSStatus.ABERTA], color: COLORS_STATUS.ABERTA }, { name: 'Análise', value: counts[OSStatus.AGUARDANDO], color: COLORS_STATUS.AGUARDANDO }, { name: 'Canceladas', value: counts[OSStatus.CANCELADA], color: COLORS_STATUS.CANCELADA }, ].filter(d => d.value > 0);
  }, [chartOrders]);
  ...
  const typeData = useMemo(() => {
      ...
      return [ { name: 'Preventiva', value: counts[OSType.PREVENTIVA], color: COLORS_TYPE[0] }, { name: 'Corretiva', value: counts[OSType.CORRETIVA], color: COLORS_TYPE[1] }, { name: 'Instalação', value: counts[OSType.INSTALACAO], color: COLORS_TYPE[2] }, { name: 'Outros', value: counts[OSType.OUTROS], color: COLORS_TYPE[3] }, ].filter(d => d.value > 0);
  }, [chartOrders]);
  ...
  const unitData = useMemo(() => {
      ...
      if (sorted.length > 6) { const top5 = sorted.slice(0, 5); const others = sorted.slice(5).reduce((acc, curr) => acc + curr.value, 0); return [...top5, { name: 'Outros', value: others, color: '#94a3b8' }]; }
      return sorted;
  }, [chartOrders]);
  ...
  const categoryData = useMemo(() => {
      const counts: Record<string, number> = {};
      filteredExpenses.forEach(e => { counts[e.category] = (counts[e.category] || 0) + e.value; });
  ...
  const SortableHeader = ({ label, sortKey, align = 'left', width, className }: { label: string, sortKey?: string, align?: 'left' | 'center' | 'right', width?: string, className?: string }) => (
    <th className={`px-6 py-4 text-xs font-bold uppercase tracking-wider transition-colors select-none ${width ? width : ''} ${align === 'center' ? 'text-center' : align === 'right' ? 'text-right' : 'text-left'} ${sortKey ? 'cursor-pointer hover:bg-slate-100/80 dark:hover:bg-slate-700/80 hover:text-indigo-600 dark:hover:text-indigo-400 group' : 'text-slate-400 dark:text-slate-500'} ${sortConfig?.key === sortKey ? 'bg-indigo-50/50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'} ${className || ''}`} onClick={() => sortKey && handleSort(sortKey)}>
        <div className={`flex items-center gap-1.5 ${align === 'center' ? 'justify-center' : align === 'right' ? 'justify-end' : 'justify-start'}`}>
            {label}
            {sortKey && (<span className={`flex flex-col ${sortConfig?.key === sortKey ? 'opacity-100' : 'opacity-0 group-hover:opacity-40'} transition-opacity`}> {sortConfig?.key === sortKey ? ( sortConfig.direction === 'asc' ? <ArrowUp size={12} /> : <ArrowDown size={12} /> ) : ( <ArrowUpDown size={12} /> )} </span>)}
        </div>
    </th>
  );
    ...
    <div className="space-y-8 pb-12 animate-in fade-in duration-500">
        ...
        <div className="flex justify-between items-start xl:items-center">
            ...
            {activeTab !== 'closed_os' && activeTab !== 'financial_records' && (
                <div className="flex flex-wrap items-center gap-3 bg-white dark:bg-slate-800 p-2 pr-3 rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] border border-slate-100 dark:border-slate-700 relative z-40">
                    ...
                    <FilterDropdown label={selectedMonths.length === 0 ? "Ano Completo" : selectedMonths.length === 1 ? SHORT_MONTHS[selectedMonths[0]] : "Meses"} count={selectedMonths.length > 0 ? selectedMonths.length : undefined} isOpen={showMonthDrop} setIsOpen={setShowMonthDrop} icon={Layers}>
                        <div className="flex flex-col">
                            <div className="p-2 border-b border-slate-50 dark:border-slate-700 flex justify-between items-center sticky top-0 bg-white dark:bg-slate-800 z-10"><span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider pl-2">Selecionar</span>{selectedMonths.length > 0 && (<button onClick={clearMonths} className="text-xs text-red-500 hover:text-red-700 dark:hover:text-red-400 font-medium px-2">Limpar</button>)}</div>
                            <div className="p-2 grid grid-cols-2 gap-1">{MONTHS_LABELS.map((m, idx) => { const isSelected = selectedMonths.includes(idx); return (<button key={m} onClick={() => toggleMonth(idx)} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-all ${isSelected ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>{isSelected ? <Check size={12} strokeWidth={3} /> : <div className="w-3" />}{m}</button>) })}</div>
                        </div>
                    </FilterDropdown>
                    <FilterDropdown label={selectedUnits.length === 0 ? "Todas Unidades" : selectedUnits.length === 1 ? selectedUnits[0] : "Unidades"} count={selectedUnits.length > 0 ? selectedUnits.length : undefined} isOpen={showUnitDrop} setIsOpen={setShowUnitDrop} icon={Filter}>
                        <div className="flex flex-col">
                            <div className="p-2 border-b border-slate-50 dark:border-slate-700 flex justify-between items-center sticky top-0 bg-white dark:bg-slate-800 z-10"><span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider pl-2">Filtrar Lojas</span>{selectedUnits.length > 0 && (<button onClick={clearUnits} className="text-xs text-red-500 hover:text-red-700 dark:hover:text-red-400 font-medium px-2">Limpar</button>)}</div>
                            <div className="p-2 space-y-1">{Object.values(Unit).map((u) => { const isSelected = selectedUnits.includes(u); return (<button key={u} onClick={() => toggleUnit(u)} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-all group ${isSelected ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 font-bold' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 font-medium'}`}><span>{u}</span>{isSelected && <Check size={14} className="text-indigo-600 dark:text-indigo-400" />}</button>) })}</div>
                        </div>
                    </FilterDropdown>
                </div>
            )}
                ...
                <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50/50 dark:bg-slate-700/50">
                        <tr>
                            ...
                            <SortableHeader label="ID" sortKey="id" className="font-bold text-slate-400 dark:text-slate-500" />
                            <SortableHeader label="Item / Serviço" sortKey="item" />
                            <SortableHeader label="Status Pag." sortKey="status" />
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                        {archivedExpenses.map(expense => {
                            ...
                            return (
                                <tr key={expense.id} onClick={() => toggleSelection(expense.id)} className={`hover:bg-slate-50/80 dark:hover:bg-slate-700/50 transition-colors group cursor-pointer ${isSelected ? 'bg-indigo-50/40 dark:bg-indigo-900/20' : ''}`}>
                                    <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                                        {!isMobile && (
                                            <input type="checkbox" className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" checked={isSelected} onChange={() => toggleSelection(expense.id)} />
                                        )}
                                    </td>
                                    <td className="px-4 py-4 font-mono text-xs text-slate-500 dark:text-slate-400">{expense.id}</td>
                                    <td className="px-4 py-4"><div className="font-bold text-slate-800 dark:text-slate-200 text-sm">{expense.item}</div><div className="text-xs text-slate-500 dark:text-slate-400">{expense.supplier}</div></td>
                                    <td className="px-4 py-4"><span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase ${expense.status === ExpenseStatus.PROGRAMADO ? 'bg-blue-50 text-blue-700 border-blue-200' : expense.status === ExpenseStatus.PAGO ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>{expense.status || 'Pendente'}</span></td>
                                    <td className="px-4 py-4 text-sm text-slate-600 dark:text-slate-300">{new Date(expense.date).toLocaleDateString('pt-BR')}</td>
                                    <td className="px-4 py-4 text-sm text-slate-600 dark:text-slate-300">{expense.unit}</td>
                                    <td className="px-4 py-4">{expense.linkedOSId ? (<span className="flex items-center gap-1 text-xs font-mono text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 px-2 py-1 rounded w-fit"><LinkIcon size={10} /> {expense.linkedOSId}</span>) : '-'}</td>
                                    <td className="px-6 py-4 text-right font-bold text-emerald-600 dark:text-emerald-400">{expense.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
                                </tr>
                            )
                        })}
                        {archivedExpenses.length === 0 && (<tr><td colSpan={8} className="px-6 py-20 text-center"><div className="flex flex-col items-center justify-center opacity-40"><div className="w-16 h-16 bg-slate-100 dark:bg-slate-700 rounded-full flex items-center justify-center mb-4"><Archive className="w-8 h-8 text-slate-400 dark:text-slate-500" /></div><p className="text-slate-600 dark:text-slate-400 font-medium">Nenhum registro encontrado.</p></div></td></tr>)}
                    </tbody>
                </table>
            </div>
        </div>
      ...
      {isSupplierListOpen && (
        <div className="fixed inset-0 bg-black/70 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[85vh] overflow-hidden border border-slate-200 dark:border-slate-700">
                {/* Modal Header */}
                <div className="p-5 border-b dark:border-slate-800 flex justify-between items-center bg-gray-50 dark:bg-slate-800/50">
                    <div>
                        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
                            <Contact className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                            Lista de Fornecedores
                        </h3>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Contatos úteis para manutenção e compras.</p>
                    </div>
                    <button onClick={() => setIsSupplierListOpen(false)} className="p-2 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-full text-gray-500 dark:text-gray-400 transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                </div>

                {/* Search Bar */}
                <div className="p-4 border-b dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 z-10">
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                        <input 
                            type="text" 
                            placeholder="Buscar por nome, categoria ou contato..." 
                            className="w-full pl-9 pr-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50 dark:bg-slate-800 text-gray-900 dark:text-white placeholder-slate-400"
                    ...
                    <div className="space-y-3">
                        {filteredSuppliers.length > 0 ? (
                            filteredSuppliers.map((sup) => (
                                <div key={sup.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors group">
                                    <div className="flex items-start gap-3">
                                        <div className="w-10 h-10 rounded-full bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shrink-0 font-bold text-sm border border-indigo-100 dark:border-indigo-800">
                                            {sup.name.charAt(0).toUpperCase()}
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-slate-800 dark:text-white text-sm">{sup.name}</h4>
                                            <div className="flex flex-wrap items-center gap-2 mt-1">
                                                <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-600">
                                                    {sup.category}
                                                </span>
                                                {sup.contactName && (
                                                    <span className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">
                                                        <UserIcon size={10} /> {sup.contactName}
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    {sup.contact && (
                                        <div className="flex items-center">
                                            <a href={`tel:${sup.contact.replace(/\D/g,'')}`} className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 rounded-lg text-xs font-bold hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors border border-emerald-100 dark:border-emerald-800">
                                                <Phone size={12} /> {sup.contact}
                                            </a>
                                        </div>
                                    )}
                                </div>
                ...
                <div className="p-4 border-t dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-end">
                    <button onClick={() => setIsSupplierListOpen(false)} className="px-6 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold rounded-xl text-sm transition-colors">
                        Fechar
                    </button>
                </div>
            </div>
