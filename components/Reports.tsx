      ...
      {activeTab === 'managerial' && (
        <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none flex flex-col items-center h-96 hover:shadow-lg transition-shadow duration-300"><h3 className="text-lg font-bold text-slate-800 dark:text-white mb-0.5 w-full text-left">Tipos de OS</h3><p className="text-xs text-slate-400 dark:text-slate-500 mb-2 uppercase tracking-wider w-full text-left">Classificação Técnica</p><div className="w-full h-[280px] mt-4"><ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={50}><PieChart><Pie data={typeData} cx="50%" cy="50%" innerRadius={65} outerRadius={85} paddingAngle={5} cornerRadius={8} dataKey="value" stroke="none">{typeData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}</Pie><Tooltip content={<CustomTooltip />} /><Legend verticalAlign="bottom" align="center" height={40} iconType="circle" iconSize={8} wrapperStyle={{ paddingTop: '20px', fontSize: '12px', fontWeight: 500, color: isDarkMode ? '#94a3b8' : '#64748b' }} /></PieChart></ResponsiveContainer><div className="absolute top-[38%] left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none"><span className="text-3xl font-extrabold text-slate-800 dark:text-white leading-none">{totalOrders}</span><span className="block text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mt-1">Total</span></div></div></div>
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none flex flex-col items-center h-96 hover:shadow-lg transition-shadow duration-300"><h3 className="text-lg font-bold text-slate-800 dark:text-white mb-0.5 w-full text-left">Volume por Loja</h3><p className="text-xs text-slate-400 dark:text-slate-500 mb-2 uppercase tracking-wider w-full text-left">Distribuição das Demandas</p><div className="w-full h-[280px] mt-4"><ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={50}><PieChart><Pie data={unitData} cx="50%" cy="50%" innerRadius={65} outerRadius={85} paddingAngle={2} cornerRadius={4} dataKey="value" stroke="none">{unitData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}</Pie><Tooltip content={<CustomTooltip />} /><Legend verticalAlign="bottom" align="center" height={40} iconType="circle" iconSize={8} wrapperStyle={{ paddingTop: '20px', fontSize: '10px', fontWeight: 500, color: isDarkMode ? '#94a3b8' : '#64748b' }} /></PieChart></ResponsiveContainer><div className="absolute top-[38%] left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none"><span className="text-3xl font-extrabold text-slate-800 dark:text-white leading-none">{totalOrders}</span><span className="block text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mt-1">Total</span></div></div></div>
            </div>
        </div>
      )}
